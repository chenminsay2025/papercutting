...
5. 12V 电源正负分别接到模块的电机输出端，再接到伸缩杆两根线。
...
> **注意：** 12V 只接 **B+**，不要接到 **VCC**。R_EN/L_EN 必须使能（接 5V），否则电机不转。
...
PA0 ──→ IN1 (或方向脚 A)
PA1 ──→ IN2 (或方向脚 B)
GND ──→ GND（与 STM32、12V 电源共地）
12V ──→ 驱动模块 motor power
驱动 OUT1/OUT2 ──→ 伸缩杆两根线
```
...
| 电源 | 用途 | 建议 |
|------|------|------|
| 12V | 伸缩杆电机 | 电流按电机铭牌，建议留 30% 余量 |
| 5V 或 3.3V | STM32 板、继电器线圈 | 可用 12V→5V→3.3V 降压模块 |
| USB | ST-LINK、USB-TTL | 各自 USB 供电即可 |

**共地规则：**

- 12V 电源负极 = 整个系统的 **GND 参考点**。
- STM32、USB-TTL、继电器、电机驱动 **必须共地**。
- **12V 正极不要** 接到 STM32 任何引脚。
...
| 电机不动 | 12V 未接、驱动使能未开 | 测 12V 输出、IN 电平 |
