...
const STEP_TYPE_META = {
  ...
  stop: { label: "停止流程" },
  buzzer: { label: "蜂鸣器", testStep: "buzzer" },
  wait_status: { label: "等待状态", testStep: "wait_status" },
  wait_obstacle: { label: "等待遮挡", testStep: "wait_obstacle" },
};
...
const STEP_DEFAULTS = {
  ...
  buzzer: { pattern: "short", on_ms: 200, gap_ms: 100, repeat: 1, duration_ms: 2000 },
  wait_status: { status_key: "obstacle", expected_value: "blocked", timeout_s: 30 },
  wait_obstacle: { expect: "blocked", timeout_s: 30 },
};

const BUZZER_PATTERNS = [
  { value: "short", label: "短鸣" },
  { value: "long", label: "长鸣" },
  { value: "double", label: "双鸣" },
  { value: "triple", label: "三鸣" },
  { value: "continuous", label: "连续" },
];

const WAIT_STATUS_KEYS = [
  { value: "paper", label: "压纸" },
  { value: "obstacle", label: "遮挡" },
];

const OBSTACLE_EXPECT_OPTIONS = [
  { value: "blocked", label: "有遮挡" },
...
function updateWaitStatusStepLabel(step) {
  if (step?.type !== "wait_status") return;
  const keyLabel = WAIT_STATUS_KEYS.find((item) => item.value === step.status_key)?.label || step.status_key;
  const valLabel = getConditionValueLabel(step.status_key, step.expected_value);
  step.label = `等待·${keyLabel}=${valLabel}`;
}
...
function normalizeWaitStatusTimeoutS(step) {
  if (step.timeout_s != null && step.timeout_s !== "") {
    return Math.max(0, Math.round(Number(step.timeout_s)) || 0);
  }
  const legacy = Number(step.timeout_ms);
  if (!Number.isFinite(legacy)) return STEP_DEFAULTS.wait_status.timeout_s;
  if (legacy >= 1000) return Math.floor(legacy / 1000);
  return Math.max(0, Math.round(legacy));
}
...
function createStep(type, config) {
  ...
  if (type === "focus_window") {
    ...
    const lastBuzzer = findLastStep("buzzer");
    step.pattern = lastBuzzer?.pattern || defaults.pattern || "short";
    step.on_ms = Number(lastBuzzer?.on_ms ?? defaults.on_ms ?? 200);
    step.gap_ms = Number(lastBuzzer?.gap_ms ?? defaults.gap_ms ?? 100);
    step.repeat = Number(lastBuzzer?.repeat ?? defaults.repeat ?? 1);
    step.duration_ms = Number(lastBuzzer?.duration_ms ?? defaults.duration_ms ?? 2000);
  } else if (type === "wait_status") {
    const lastWait = findLastStep("wait_status");
    step.status_key = lastWait?.status_key || defaults.status_key || "obstacle";
    const options = CONDITION_STATUS_OPTIONS[step.status_key] || CONDITION_STATUS_OPTIONS.obstacle;
    step.expected_value = lastWait?.expected_value || defaults.expected_value || options[0]?.value || "blocked";
    step.timeout_s = normalizeWaitStatusTimeoutS(lastWait || defaults);
    updateWaitStatusStepLabel(step);
  } else {
...
function normalizeWorkflowSteps(steps, config) {
  ...
  return syncPulseStepLabels(
    ...
    let step = rawStep;
    let type = step.type;
    if (type === "wait_obstacle") {
      step = {
        ...step,
        type: "wait_status",
        status_key: "obstacle",
        expected_value: String(step.expect || "blocked").trim().toLowerCase(),
      };
      type = "wait_status";
    }
    if (type === "cut_wait") {
    ...
    if (type === "focus_window") {
      ...
      const pattern = String(step.pattern || STEP_DEFAULTS.buzzer.pattern).trim().toLowerCase();
      normalized.pattern = BUZZER_PATTERNS.some((item) => item.value === pattern)
        ? pattern
        : STEP_DEFAULTS.buzzer.pattern;
      normalized.on_ms = Number(step.on_ms ?? STEP_DEFAULTS.buzzer.on_ms);
      normalized.gap_ms = Number(step.gap_ms ?? STEP_DEFAULTS.buzzer.gap_ms);
      normalized.repeat = Number(step.repeat ?? STEP_DEFAULTS.buzzer.repeat);
      normalized.duration_ms = Number(step.duration_ms ?? STEP_DEFAULTS.buzzer.duration_ms);
    } else if (type === "wait_status") {
      const statusKey = String(step.status_key || STEP_DEFAULTS.wait_status.status_key).trim();
      normalized.status_key = WAIT_STATUS_KEYS.some((item) => item.value === statusKey)
        ? statusKey
        : STEP_DEFAULTS.wait_status.status_key;
      const options = CONDITION_STATUS_OPTIONS[normalized.status_key] || CONDITION_STATUS_OPTIONS.obstacle;
      normalized.expected_value = String(step.expected_value || STEP_DEFAULTS.wait_status.expected_value).trim();
      if (!options.some((item) => item.value === normalized.expected_value)) {
        normalized.expected_value = options[0]?.value || STEP_DEFAULTS.wait_status.expected_value;
      }
      normalized.timeout_s = normalizeWaitStatusTimeoutS(step);
      updateWaitStatusStepLabel(normalized);
...
function buzzerActionDurationMs(step) {
  const pattern = String(step.pattern || "short").trim().toLowerCase();
  const onMs = Math.max(0, Number(step.on_ms) || 200);
  const gapMs = Math.max(0, Number(step.gap_ms) || 100);
  const repeat = Math.max(1, Number(step.repeat) || 1);
  if (pattern === "short" || pattern === "long") return onMs;
  if (pattern === "double") return onMs * 2 + gapMs;
  if (pattern === "triple") return onMs * 3 + gapMs * 2;
  if (pattern === "continuous") return Math.max(onMs, Number(step.duration_ms) || 2000);
  return onMs * repeat + gapMs * Math.max(0, repeat - 1);
}
...
function stepDurationMs(step) {
  ...
  if (step.type === "focus_window") {
    ...
    actionMs = 0;
  } else if (step.type === "wait_status" || step.type === "wait_obstacle") {
    actionMs = 0;
  } else if (step.type === "buzzer") {
    actionMs = buzzerActionDurationMs(step);
...
function renderStepParams(step, index, canEdit) {
  ...
  if (step.type === "focus_window") {
    ...
    const showContinuous = step.pattern === "continuous";
    inner = `
      ${renderInlineSelect("节奏", "pattern", step, index, canEdit, BUZZER_PATTERNS)}
      ${renderInlineNumber("响", "on_ms", step, index, canEdit, 0, 50)}
      ${renderInlineNumber("间隔", "gap_ms", step, index, canEdit, 0, 50)}
      ${showContinuous ? renderInlineNumber("时长", "duration_ms", step, index, canEdit, 0, 100) : renderInlineNumber("次数", "repeat", step, index, canEdit, 1, 1)}
    `;
  } else if (step.type === "wait_status") {
    const statusKey = step.status_key || "obstacle";
    const valueOptions = CONDITION_STATUS_OPTIONS[statusKey] || CONDITION_STATUS_OPTIONS.obstacle;
    inner = `
      ${renderInlineSelect("状态", "status_key", step, index, canEdit, WAIT_STATUS_KEYS)}
      ${renderInlineSelect("等待", "expected_value", step, index, canEdit, valueOptions)}
      ${renderInlineNumber("超时(s)", "timeout_s", step, index, canEdit, 0, 1, "0 表示不限时")}
    `;
  ...
  const skipDelayBadge = new Set([
    "condition_check", "else_branch", "end_if", "call_group", "stop", "wait_status",
  ]);
...
const WAIT_TIMER_PHASES = new Set(["wait_status", "wait_obstacle", "confirm_dialog"]);

function syncWaitTimerFromPayload(payload) {
  const phase = String(payload.phase || "");
  if (!WAIT_TIMER_PHASES.has(phase)) {
    state.waitTimerActive = false;
    state.waitElapsedMs = 0;
    state.waitTotalMs = 0;
    return;
  }
  const message = String(payload.message || "");
  const stepDone =
    Number(payload.step_progress) >= 1 ||
    (message.length > 0 && !message.startsWith("等待"));
  if (stepDone) {
    state.waitTimerActive = false;
    state.waitElapsedMs = 0;
    state.waitTotalMs = 0;
  ...
  state.waitTimerActive = true;
  state.waitElapsedMs = Math.max(0, Number(payload.step_elapsed_ms) || 0);
  state.waitTotalMs = Math.max(0, Number(payload.step_total_ms) || 0);
}
...
function handleStepRunUpdate(payload) {
  ...
  syncWaitTimerFromPayload(payload);
  if (stepChanged) {
  ...
  if (!state.waitingLoop) {
    ...
    const hasCycleElapsed =
      payload.elapsed_ms !== undefined && payload.total_ms !== undefined;
    if (hasCycleProgress || hasCycleElapsed) {
      updateProgress(
        payload.elapsed_ms ?? 0,
        payload.total_ms ?? state.totalMs,
        payload.message || payload.phase_label || "运行中",
        hasCycleProgress ? payload.progress : undefined
      );
    }
  }
}
...
function parseObstaclePosition(raw) {
  const upper = String(raw || "").toUpperCase();
  if (upper.includes("OBSTACLE:BLOCKED")) return "blocked";
  if (upper.includes("OBSTACLE:CLEAR")) return "clear";
  return null;
}
...
function buildStatusSnapshot() {
  return {
    ...
    obstaclePosition: state.obstaclePosition,
    ...
    waitTimerActive: state.waitTimerActive,
    waitElapsedMs: state.waitElapsedMs,
    waitTotalMs: state.waitTotalMs,
...
function readWorkflowStepsFromState() {
  return syncPulseStepLabels(state.workflowSteps, state.config).map((step) => {
    ...
    if (step.type === "focus_window") {
      ...
      copy.pattern = String(step.pattern || "short").trim().toLowerCase();
      copy.on_ms = Math.max(0, Number(step.on_ms) || 200);
      copy.gap_ms = Math.max(0, Number(step.gap_ms) || 100);
      copy.repeat = Math.max(1, Number(step.repeat) || 1);
      copy.duration_ms = Math.max(0, Number(step.duration_ms) || 2000);
    } else if (step.type === "wait_status") {
      copy.status_key = String(step.status_key || "obstacle").trim();
      copy.expected_value = String(step.expected_value || "blocked").trim();
      copy.timeout_s = normalizeWaitStatusTimeoutS(step);
...
function updateProgress(elapsedMs, totalMs, label, progressRatio) {
  ...
  let ratio =
    progressRatio != null
      ...
      : total > 0
        ? Math.min(1, Math.max(0, Number(elapsedMs) / total))
        : 0;
  if (state.running) {
    ratio = Math.max(state.progressRatio ?? 0, ratio);
  }
  state.progressElapsed = total > 0 ? Math.min(total, Math.round(ratio * total)) : Math.max(0, Number(elapsedMs) || 0);
...
let lcdStepSyncTimer = null;

async function syncLcdStepsNow() {
  if (!state.connected || state.running || !state.pythonReady) return;
  try {
    await sendCommand({
      cmd: "sync_lcd_steps",
      workflow_steps: readWorkflowStepsFromState(),
    ...
    // ignore transient serial errors during live sync
  }
}

function scheduleLcdStepSync() {
  if (!state.connected || state.running) return;
  clearTimeout(lcdStepSyncTimer);
  lcdStepSyncTimer = setTimeout(() => {
    lcdStepSyncTimer = null;
    void syncLcdStepsNow();
  }, 250);
}
...
function handleBackendEvent(payload) {
  switch (payload.event) {
    ...
    case "device_status":
      ...
      if (payload.obstacle) state.obstaclePosition = payload.obstacle;
      else if (payload.raw) state.obstaclePosition = parseObstaclePosition(payload.raw);
...
function handleStepFieldInput(target) {
  ...
  if (field === "status_key") {
    ...
    if (state.workflowSteps[index].type === "wait_status") {
      updateWaitStatusStepLabel(state.workflowSteps[index]);
    } else {
...
els.resetLoopBtn?.addEventListener("click", async (e) => {
  e.stopPropagation();
  closeAllMenus();
  try {
    await sendCommand({ cmd: "reset_lcd_loop" });
    state.loopIndex = 0;
    if (!state.running) {
      updateProgress(0, state.totalMs, "等待开始", 0);
    }
    log("info", "LCD 轮次已清零");
  } catch (err) {
    log("error", err.message);
