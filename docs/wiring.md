...
> **默认硬件：nologo STM32F103C8T6/C6T6 + 0.96" TFT 一体板**（板载 LCD / Flash，见 §5.8）  
> 烧录：ST-LINK V2（SWD）  
> 电脑控制：USB-TTL 模块（3.3V 电平）

**可视化版本（推荐）：** 用浏览器打开 [`wiring-diagram.html`](wiring-diagram.html)  
接线总表：[`wiring-table.md`](wiring-table.md) · [`wiring-table.xlsx`](wiring-table.xlsx)
...
| **PA4** | LED 缩回 | D1 → 330Ω → GND | 缩回时常亮 |
| **PA5** | LED 伸出 | D2 → 330Ω → GND | 伸出时常亮 |
| **PA6** | LCD 背光 *或* 串口 LED | 见 §5.8 | nologo：**PA6=背光（低电平亮）**；标准板：**PA6=D3 串口灯** |
| **PA7** | LCD RES *或* 切换按键 | 见 §5.8 | nologo：**PA7=LCD 复位**；标准板：**PA7=切换键→GND** |
| **PA8** | 槽型光电 DO | 压纸检测模块 **DO** | 遮挡=压纸中（`ROD:HOME`）；未遮挡=未压纸（`ROD:AWAY`） |
| **PA9** | USART1_TX | USB-TTL **RX** | 交叉连接 |
| **PA10** | USART1_RX | USB-TTL **TX** | 交叉连接 |
| **PB8** | 伸缩切换键 | 按钮 → GND | **nologo 一体板**；标准板用 PA7 |
| **PB9** | 串口状态 LED | D3 → 330Ω → GND | **nologo 一体板**；标准板用 PA6 |
| **PB0~PB1, PB10~PB11** | 板载 LCD | ST7735 FPC | nologo 已焊；DC/CS/SCL/SDA |
| **PB12~PB15** | 板载 Flash | W25Q64 SPI2 | 固件勿占用 |
...
- **串口状态 LED（D3）**：nologo 一体板接 **PB9**；标准最小系统板接 **PA6**。快闪 = 未连接/无通信；连接成功后约每 1.5 秒 PING 一次，变为**慢呼吸**。LCD 屏「连接」行同步显示。
...
### 5.4 伸缩杆实体按键（PA7 或 PB8）

**通用外接板：** PA7 → 切换按钮 → GND

**nologo 0.96 TFT 一体板：** PA7 被 LCD 占用，按键改 **PB8 → GND**

现场手动控制伸缩杆，与电脑串口命令可并存：

```
PA7 或 PB8 ── 切换按钮 ── GND   （nologo 一体板用 PB8）
```

| 引脚 | 功能 | 操作 |
|------|------|------|
| **PA7** / **PB8** | 伸缩切换 | **按一次**缩回 3 秒并自动停止；**再按一次**伸出 3 秒并自动停止，交替循环 |

- 固件内部上拉，按下为低电平；建议用常开轻触开关或面板按钮。
- 每次动作固定 **3 秒**（`BUTTON_ACTION_MS`），运行中再次按键无效。
- 实体键仅在**本键曾驱动电机**时到时才发 `STOP`，不影响电脑下发的 `RETRACT`/`EXTEND` 保持运行。

### 5.5 槽型光电传感器（PA8 · 压纸检测）

使用 **U 型槽型光电模块**（LM393，VCC/GND/DO，**AO 不接**）检测伸缩杆是否缩回到位（压纸状态）：
...
| 端子 | 接法 | 说明 |
|------|------|------|
| **VCC** | STM32 **3.3V** | 模块 3.3~5V，接 3.3V |
| **GND** | **GND** | 与系统共地 |
| **DO** | **PA8** | 有遮挡（挡片在槽内）→ **压纸中** `ROD:HOME` |
| **AO** | **不接** | 本模块 AO 无作用 |

- 传感器装在 **缩回终点**；伸缩杆上装 **挡片**，缩回到位时挡片进入 U 型槽。
- **遮挡 = 压纸中**（已缩回，`ROD:HOME`）；**未遮挡 = 未压纸**（已伸出或中间位，`ROD:AWAY`）。
- 缩回运行中检测到 HOME 时，固件 **自动停止电机**。
- 串口：`ROD_SENSOR` → `ROD:HOME`/`ROD:AWAY`；`STATUS` 行末附带 `;ROD:…`。
- LCD 与上位机同步显示「压纸中 / 未压纸」；模块电位器调灵敏度；遮挡时「开关指示」亮属正常。
...
5. 安装 PA8 槽型光电于缩回终点，串口发 `ROD_SENSOR` 验证。

### 5.8 板载 0.96" TFT（nologo 一体板 · ST7735）

适用于 [nologo STM32F103C8T6/C6T6 + 0.96 TFT](https://www.nologo.tech/product/stm32/STM32F103C8T6-C6T60.96TFT/stm32f103rct61.html)（屏已焊在板上，无需外接线）。显示例程见 [官方例程 7](https://www.nologo.tech/product/stm32/STM32F103C8T6-C6T60.96TFT/example/example7.html)。

**LCD FPC → MCU（与 `屏幕显示_源码示例/HARDWARE/LCD/lcd_init.h` 一致）：**
...
| 4 | **PB11** | SDA（MOSI） |
| — | **PA7** | RES（复位） |
| 8 | LEDA | 背光，由 **PA6** 经三极管驱动（低电平点亮） |

**板载 8MB Flash（W25Q64）占用 PB12~PB15（SPI2），固件勿改这些脚。**

**与 CutPPaper 外设的差异（重要）：**

| 原设计 | nologo 板 |
|--------|-----------|
| PA6 = 串口状态 LED | **PA6 = LCD 背光**；串口灯改 **PB9 → D3** |
| PA7 = 切换按键 | **PA7 = LCD 复位(RES)**，按键改接 **PB8 → GND** |
| PA4/PA5 外接 LED | 仍可用（缩回/伸出指示） |

固件配置：`firmware/User/lcd_config.h`（`BOARD_NOLOGO_096TFT = 1`）。
