...
Build with Keil5: open `firmware/CutPPaper.uvprojx`, **Build (F7) first**, then flash via ST-LINK (F8). Output is `firmware/Objects/CutPPaper.axf` — if missing, Build failed (check `firmware/Objects/CutPPaper.build_log.htm`). After LCD Font Studio export or firmware edits, Rebuild before Download.
