...
> **硬件：标准 STM32F103C8T6 最小系统板 + 外接 0.96" SPI OLED（SSD1306）**  
> 烧录：ST-LINK V2（SWD）  
...
flowchart TB
    ...
    subgraph MCU["STM32F103C8T6"]
        ...
        PA9["PA9 TX"]
        PA10["PA10 RX"]
        OLED["PB8/PB9/PB13/PB14 OLED"]
    end

    subgraph DISPLAY["状态显示"]
        SCR["0.96\" SPI OLED SSD1306"]
    end
...
| **PA6** | 串口状态 LED | D3 → 330Ω → GND | 快闪=未连接；慢呼吸=已连接 |
| **PA7** | 伸缩切换按键 | 按钮 → GND | 按一次缩回 3s，再按伸出 3s，交替 |
| **PA8** | 槽型光电 DO | 压纸检测模块 **DO** | 遮挡=压纸中（`ROD:HOME`）；未遮挡=未压纸（`ROD:AWAY`） |
    | **PA9** | USART1_TX | USB-TTL **RX** | 交叉连接 |
    | **PA10** | USART1_RX | USB-TTL **TX** | 交叉连接 |
| **PB8** | OLED SCL | 外接 SSD1306 **SCL/CLK** | 软件 SPI 时钟 |
| **PB9** | OLED SDA | 外接 SSD1306 **SDA/DIN** | 软件 SPI 数据 |
| **PB13** | OLED DC | 外接 SSD1306 **DC** | 数据/命令 |
| **PB14** | OLED CS | 外接 SSD1306 **CS** | 片选 |
| **PA13** | SWDIO | ST-LINK SWDIO | 烧录/调试 |
| **PA14** | SWCLK | ST-LINK SWCLK | 烧录/调试 |
| **3.3V** | 电源 | ST-LINK 3.3V（可选） | 建议板子独立供电；OLED VCC、光电 VCC |
...
- **串口状态 LED（D3）**：接 **PA6**。快闪 = 未连接/无通信；连接成功后约每 1.5 秒 PING 一次，变为**慢呼吸**。外接 OLED 屏「USB」行同步显示。
...
- 外接 OLED 与上位机同步显示「压纸中 / 未压纸」；模块电位器调灵敏度；遮挡时「开关指示」亮属正常。
...
### 5.8 外接 0.96" SPI OLED（SSD1306 · 7 针）

外接 OLED 为现场状态屏。引脚与示例 `SPI接口液晶显示中文字符串数字(标准库)` 一致：
...
OLED SCL ── PB8
OLED SDA ── PB9
OLED RES ── 3.3V          （模块 RST 直连，固件不占 RST 脚）
OLED DC  ── PB13
OLED CS  ── PB14
```
...
| GND | GND | 共地 |
| VCC | 3.3V | 勿接 5V |
| SCL / CLK | **PB8** | SPI 时钟（软件 SPI） |
| SDA / DIN | **PB9** | SPI 数据 |
| RES | 3.3V | 模块 RST 接 3.3V |
| DC | **PB13** | 数据/命令 |
| CS | **PB14** | 片选 |

**同板其它外设：** PA6=串口状态灯 D3，PA7=切换键→GND（与 OLED 引脚无冲突）。
...
固件：`firmware/User/oled_config.h`、`oled_ui.c`。
...
4. 接 **外接 OLED**（§5.8）与 **PA8 槽型光电**，上电后屏应显示状态行。
...
└─────────────────────────────────────┼─────────────────────────────┼───────┘
                    ...
                    ┌─────────────────▼─────────────────────────────▼───────┐
                    │              STM32F103C8T6 最小系统板                    │
                    │  PA0~PA3 继电器  PA6 D3  PA7 切换键  PA8 压纸            │
                    │  PA9/10 串口  PA13/14 SWD  PB8/PB9/PB13/PB14 → OLED     │
                    └──┬────┬─────────┬───┬───┬───┬───┬──────────┬───────────┘
                       │    │         │   │   │   │   │          │
              USB-TTL ─┘    └─ USB-TTL│   │   │   │   │          └─ 0.96" OLED
                                        │   │   │   │   │
                    ┌───────────────────┘   │   │   └──────────────┐
                    ▼                       ▼   ▼                  ▼
            ┌─────────────────────────────────────────┐
...
| STM32 信号 | 杜邦线 | PA0~PA3、串口、SWD、OLED（PB8/PB9/PB13/PB14） |
...
4. 接 **外接 OLED**（§5.8），确认上电有显示。
5. **不接电机**，PA0/PA1 单步测试，听 K1/K2 吸合、测触点通断。
6. 接 **24V**，伸缩杆 **空载短行程** 试缩回/伸出。
7. K3/K4 **不接机器**，测 NO3/NO4 通断。
8. 确认按钮为 **干触点后**，再将 NO 并联到「继续」「原点」。
9. 电脑运行 CutPPaper，**模拟模式关闭**，完整联调。
