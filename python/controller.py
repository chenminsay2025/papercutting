...
class ControllerService:
    ...
    def _handle_link_lost(self, reason: str) -> None:
        if self._using_mock or not self._link_active:
            return
        with self._client_lock:
            ...
            if self.client is not None:
                try:
                    ...
                    pass
        msg = reason if reason else "USB 串口已断开"
        self.emit({"event": "disconnected", "reason": msg})
        self.emit({"event": "log", "level": "warn", "message": msg})
    ...
    def _start_heartbeat(self) -> None:
        def loop() -> None:
            while not self._heartbeat_stop.wait(self._heartbeat_interval_s):
                try:
                    with self._client_lock:
                        if self._using_mock or self.client is None or not self._link_active:
                            self._heartbeat_failures = 0
                            continue
                        if not self.client.connected:
                            self._handle_link_lost("串口连接已丢失")
                            continue
                        port = getattr(self.client, "port", "")
                        if port and not Stm32Client.is_port_available(port):
                            self._handle_link_lost("USB 设备已拔出")
                            continue
                        self.client.ping()
                    ...
                    drain = getattr(self.client, "drain_ok_lines", None)
                    if callable(drain):
                        drain(4)
                    ...
                    if isinstance(exc, TimeoutError) and self._heartbeat_failures < 2:
                        continue
                    self._handle_link_lost(str(exc) or "串口通信失败")
        ...
        try:
            ...
            if cmd == "save_config":
                ...
                if not self.workflow.running:
                    with self._client_lock:
                        self._apply_client_settings()
                    self.workflow.sync_lcd_steps(self.config)
            ...
            if cmd == "start_cycle":
                ...
                with self._client_lock:
                    if self.client is not None and self.client.connected:
                        try:
                            self.client.ui_progress_clear()
                            self.client.ui_progress(0)
                        except Exception:
                            pass
            ...
            if cmd == "reset_lcd_loop":
                with self._client_lock:
                    if self.client is None or not self.client.connected:
                        raise RuntimeError("请先连接串口")
                    self.client.ui_loop_reset()
                reply({"event": "lcd_loop_reset"})
                return

            if cmd == "sync_lcd_steps":
                if self.workflow.running:
                    raise RuntimeError("流程运行中，无法同步 LCD 动作列表")
                incoming_steps = message.get("workflow_steps")
                cfg = self.config
                if incoming_steps is not None:
                    cfg = self._merge_config({"workflow_steps": incoming_steps})
                self.workflow.sync_lcd_steps(cfg)
                reply({"event": "lcd_steps_synced"})
                return
