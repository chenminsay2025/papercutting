...
CONDITION_STATUS_KEYS = ("paper", "motor", "usb", "obstacle")

WAIT_STATUS_KEYS = ("paper", "obstacle")
...
WAIT_STATUS_DEFAULTS: dict[str, Any] = {
    "status_key": "obstacle",
...
def wait_status_label(status_key: str, expected_value: str) -> str:
    key_label = CONDITION_STATUS_LABELS.get(status_key, status_key)
    val_label = condition_value_label(status_key, expected_value)
    return f"等待·{key_label}={val_label}"


def wait_status_timeout_s_from_raw(raw: dict[str, Any]) -> int:
    """等待状态超时（秒）。0 表示不限时。兼容 wait_obstacle / timeout_ms。"""
    if raw.get("timeout_s") is not None:
        return max(0, int(raw["timeout_s"]))
    legacy = raw.get("timeout_ms")
    if legacy is None:
        return int(WAIT_STATUS_DEFAULTS["timeout_s"])
    legacy = int(legacy)
    if legacy >= 1000:
        return legacy // 1000
    return max(0, legacy)
...
def wait_status_timeout_ms(step: dict[str, Any]) -> int:
    return wait_status_timeout_s_from_raw(step) * 1000


# 兼容旧名
WAIT_OBSTACLE_DEFAULTS = WAIT_STATUS_DEFAULTS
wait_obstacle_timeout_s_from_raw = wait_status_timeout_s_from_raw
wait_obstacle_timeout_ms = wait_status_timeout_ms
    ...
    if steps is None:
        ...
        for index, step in enumerate(steps):
            ...
            if step_type == "focus_window":
                ...
                pattern = str(step.get("pattern", "")).strip().lower()
                if pattern not in ("short", "long", "double", "triple", "continuous"):
                    errors.append(f"{prefix} pattern 无效: {pattern}")
                for field in ("on_ms", "gap_ms", "duration_ms"):
                    value = step.get(field)
                    if value is not None and (
                        not isinstance(value, int) or not (0 <= value <= MAX_TIMING_MS)
                    ):
                        errors.append(f"{prefix} {field} 必须是 0–{MAX_TIMING_MS} 的整数")
                repeat = step.get("repeat")
                if repeat is not None and (
                    not isinstance(repeat, int) or not (1 <= repeat <= 20)
                ):
                    errors.append(f"{prefix} repeat 必须是 1–20 的整数")
            elif step_type in ("wait_status", "wait_obstacle"):
                if step_type == "wait_obstacle":
                    status_key = "obstacle"
                    expected_value = str(step.get("expect", "")).strip().lower()
                else:
                    status_key = str(step.get("status_key", "")).strip()
                    expected_value = str(step.get("expected_value", "")).strip()
                if status_key not in WAIT_STATUS_KEYS:
                    errors.append(f"{prefix} status_key 无效: {status_key}")
                elif expected_value not in CONDITION_STATUS_VALUES.get(status_key, ()):
                    errors.append(f"{prefix} expected_value 无效: {expected_value}")
                timeout_s = step.get("timeout_s")
                if timeout_s is None and "timeout_ms" in step:
                    try:
                        timeout_s = wait_status_timeout_s_from_raw(step)
                    except (TypeError, ValueError):
                        timeout_s = None
                if timeout_s is None:
                    timeout_s = WAIT_STATUS_DEFAULTS["timeout_s"]
                if not isinstance(timeout_s, int) or timeout_s < 0:
                    errors.append(f"{prefix} timeout_s 必须是不小于 0 的整数（0 表示不限时）")
...
def normalize_workflow_steps(
    ...
    for raw in source:
        ...
        if step_type == "wait_obstacle":
            raw = {
                **raw,
                "type": "wait_status",
                "status_key": "obstacle",
                "expected_value": str(raw.get("expect") or "blocked").strip().lower(),
            }
            step_type = "wait_status"
        ...
        if step_type == "focus_window":
            ...
            pattern = str(raw.get("pattern") or BUZZER_DEFAULTS["pattern"]).strip().lower()
            if pattern not in ("short", "long", "double", "triple", "continuous"):
                pattern = BUZZER_DEFAULTS["pattern"]
            item["pattern"] = pattern
            item["on_ms"] = int(raw.get("on_ms", BUZZER_DEFAULTS["on_ms"]))
            item["gap_ms"] = int(raw.get("gap_ms", BUZZER_DEFAULTS["gap_ms"]))
            item["repeat"] = int(raw.get("repeat", BUZZER_DEFAULTS["repeat"]))
            item["duration_ms"] = int(raw.get("duration_ms", BUZZER_DEFAULTS["duration_ms"]))
        elif step_type == "wait_status":
            status_key = str(raw.get("status_key") or WAIT_STATUS_DEFAULTS["status_key"]).strip()
            if status_key not in WAIT_STATUS_KEYS:
                status_key = WAIT_STATUS_DEFAULTS["status_key"]
            allowed = CONDITION_STATUS_VALUES.get(status_key, ())
            expected_value = str(raw.get("expected_value") or WAIT_STATUS_DEFAULTS["expected_value"]).strip()
            if expected_value not in allowed:
                expected_value = allowed[0] if allowed else WAIT_STATUS_DEFAULTS["expected_value"]
            item["status_key"] = status_key
            item["expected_value"] = expected_value
            item["timeout_s"] = wait_status_timeout_s_from_raw(raw)
            item["label"] = wait_status_label(status_key, expected_value)
        elif step_type in STEP_DURATION_DEFAULTS:
